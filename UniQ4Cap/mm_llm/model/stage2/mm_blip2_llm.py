...
from lavis.models.blip2_models.mm2_blip2 import Blip2Base
...
from mm_llm.model.mm_blip2_models.mm_blip2 import Blip2Base
...
class Blip2VicunaXInstruct(Blip2Base):
    ...
    def __init__(
            ...
            video_encoder_pretrained="",
            audio_encoder_pretrained="",
        ...
        self.video_encoder_pretrained = video_encoder_pretrained
        self.audio_encoder_pretrained = audio_encoder_pretrained
        self.video_config = vit_config
        self.audio_config = aud_config
        self.shared_qformer = shared_qformer
        qformer_embed_dim = 1408

        for modality in self.modalities:
            init_method = getattr(self,f'init_{modality}_encoder')
            encoder, ln_encoder = init_method(
                encoder_pretrained=getattr(self,f'{modality}_encoder_pretrained'),
                config=getattr(self,f'{modality}_config'),
                freeze_encoder=freeze_encoder
            )
            setattr(self,f'{modality}_encoder',encoder)
            setattr(self,f'ln_{modality}',ln_encoder)
            setattr(self,f'{modality}_project',nn.Linear(getattr(self,f'{modality}_embed_dim'),qformer_embed_dim))
        ...
        if self.shared_qformer:
            ...
            self.Qformer, self.query_tokens = self.init_Qformer(
                ...
                feature_width=qformer_embed_dim,
            ...
            for modality in modalities:
                tokenizer = self.init_tokenizer(bert_pretrained)
                Qformer, query_tokens = self.init_Qformer(
                    num_query_token=num_query_token,
                    feature_width=qformer_embed_dim,
                    cross_attention_freq=cross_attention_freq,
                    bert_pretrained=bert_pretrained,
                ...
                setattr(self, f'{modality}_tokenizer', tokenizer)
                setattr(self, f'{modality}_Qformer', Qformer)
                setattr(self, f'{modality}_query_tokens', query_tokens)
        ...
        if self.shared_qformer:
            ...
            for modality in modalities:
                setattr(self,f'{modality}_llm_project',nn.Linear(768,self.llm_hidden_size))
    ...
    def forward(self, samples):
        ...
        text = samples["text"]
        for modality in self.modalities:
            data = samples[modality].to(device)
            ln = getattr(self, f"ln_{modality}")
            encoder = getattr(self, f"{modality}_encoder")
            project = getattr(self,f"{modality}_project")
            embeds[modality] = project(ln(encoder(data)))
        ...
        if self.shared_qformer:
            ...
            concat_embds = torch.cat([embed for embed in embeds.values()],dim=1)
            ...
            inputs_llms['share'] = self.llm_projection(query_output)
            atts_llms['share'] = torch.ones(inputs_llms['share'].size()[:-1],dtype=torch.long).to(self.device)
        else:
            for modality in self.modalities:
                if modality in samples.keys():
                    query_tokens = getattr(self,f'{modality}_query_tokens').expand(len(text),-1,-1)
                    embed = embeds[modality]
                    atts = torch.ones(embed.size()[:-1], dtype=torch.long).to(self.device)
                    ...
                    query_output = query_output.last_hidden_state
                    project = getattr(self,f'{modality}_llm_project')
                    inputs_llms[modality] = project(query_output)
                    atts_llms[modality] = torch.ones(inputs_llms[modality].size()[:-1],dtype=torch.long).to(self.device)

        inputs_llm = torch.cat([input_llm for input_llm in inputs_llms.values()],dim=1)
        atts_llm = torch.cat([att_llm for att_llm in atts_llms.values()],dim=1)
    ...
    def _predict_class(
        ...
        if 'discrn' in samples and self.use_caption:  ## discriminatory reasoning
                ...
                if modality == "video" and "clip" in self.video_enc_name:
                    ...
                    for j in range(data.size(2)):
                        ...
                        with self.maybe_autocast():
                            embeds[modality].append(ln(encoder(this_frame)))
                            if self.shared_qformer:
                                embeds[modality][j] = getattr(self, f"{modality}_encoder_projection")(
                                    embeds[modality][j])
                            data_atts[modality].append(
                                torch.ones(embeds[modality][j].size()[:-1], dtype=torch.long).to(self.device))
            ...
            if self.qformer_text_input:
                for modality in curr_modalities:
                    if modality in Blip2VicunaXInstruct.SEQUENCIAL_MODALITIES and getattr(self,
                        ...
                        num[modality] = len(embeds[modality])
                        bs = embeds[modality][0].shape[0]
                        indices = [j_ + r for r, j in enumerate([[i * bs for i in range(num[modality])]] * bs) for j_ in
                                   j]
                        reordered_embeds = torch.cat(embeds[modality])[indices]
                        reordered_atts = torch.cat(data_atts[modality])[indices]
                        if self.projection_only or getattr(self, f"projection_only_{modality}"):
                            if self.proj_dim != 1:
                                query_outputs[modality] = getattr(self, f"{modality}_projection")(
                                    reordered_embeds.mean(1, keepdim=True)).view(bs * num[modality],
            ...
            for i, modality in enumerate(curr_modalities):
                if modality in Blip2VicunaXInstruct.SEQUENCIAL_MODALITIES and getattr(self,
                    ...
                    if self.projection_only or getattr(self, f"projection_only_{modality}"):
                        if self.proj_dim != 1:
                            inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(
                                query_outputs[modality].unsqueeze(1)).reshape(bs * num[modality], self.num_query_token,
                                                                              -1)
                        else:
                            inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(
                                query_outputs[modality].reshape(bs * num, self.num_query_token, -1))
                        inputs_llm[modality] = inputs_llm[modality].reshape(bs, num[modality], self.num_query_token,
                                                                            -1).view(bs, num[
                            modality] * self.num_query_token, -1)
                        atts_llm[modality] = torch.ones(inputs_llm[modality].size()[:-1], dtype=torch.long).to(
                            self.device)
                        continue
                    # num*bs, num query tokens, llm emb size
                    inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(
            ...
            if self.prefix:
                att_list = [self.tokenized_prefix.attention_mask.repeat(bs, 1).to(self.device)]
                inp_list = [
                    self.llm_model.get_input_embeddings()(self.tokenized_prefix.input_ids.to(self.device)).repeat(bs, 1,
                                                                                                                  1)]
        ...
        return {"predictions": all_losses,
                "targets": torch.tensor([self.candidates.index(l) for l in samples["label"]])}
    ...
    def from_config(cls, cfg):
        ...
        model = cls(
            ...
            video_encoder_pretrained=vit_encoder_pretrained_ckpt,
            audio_encoder_pretrained=aud_encoder_pretrained_ckpt,
            bert_pretrained=bert_pretrained,
    ...
    def init_video_encoder(self,encoder_pretrained='',config=None,freeze_encoder=True):
        if self.pre_align:
            video_encoder = ClipVideo(encoder_pretrained,config,freeze_encoder).model
        else:
            ...
            video_encoder.load_state_dict(torch.load(encoder_pretrained))
            if freeze_encoder:
    ...
    def init_audio_encoder(self,encoder_pretrained='',config=None,freeze_encoder=True):
        if self.pre_align:
            audio_encoder = ClipAudio(encoder_pretrained,config,freeze_encoder).model
        else:
            audio_encoder = BeatsEncoder(encoder_pretrained)
            if freeze_encoder:
    ...
    @classmethod
    def init_Qformer(cls, num_query_token, feature_width, cross_attention_freq=2, bert_pretrained=None,
                     pretrained_qformer=None, load_attention=False):
